...
const onSubmit = async () => {
    const valid = await form.value?.validate({ silent: true }) as unknown as Promise<typeof schema | boolean>;
    if (!valid) {
        toast.add({
            title: 'Login Fail',
...
<template>
    <div class="flex flex-col justify-center items-center h-screen">
        <div
            class="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-[500px] w-[500px] filter blur-xl rounded-full bg-white/10" />
        <div>
            <div class="flex justify-between items-center px-4">
                <h1 class="py-4">{{ '\<' }} Login {{ '/\>' }}</h1>
                <ULink as="nuxt-link" to="/" class="transition-colors">
                    <NBLogo :width="30" :height="25" />
                </ULink>
            </div>

            <div
                class="relative z-1 flex flex-col space-y-4 w-[400px] px-4 py-8 rounded-2xl bg-[var(--ui-bg)] shadow-lg ring-1 ring-black/5">
                <UForm ref="form" :schema="schema" :state="model" class="flex flex-col space-y-3 text-neutral-900">
                    <UFormField label="Email" name="email">
                        <UInput v-model="model.email" label="Email" size="xl" class="w-full text-neutral-900" />
                    </UFormField>
                    <UFormField label="Password" name="password">
                        <UInput v-model="model.password" label="Password" :type="passwordType" size="xl" class="w-full">
                            <template #trailing>
                    ...
                    <div class="flex justify-between space-x-4 pt-2 pl-2">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <UButton @click="onSubmit">
                                Let's go
                                <UIcon name="material-symbols:arrow-forward-rounded" size="24"
                ...
                <div class="flex justify-center space-x-4">
                    <UButton icon="logos:google-icon" variant="outline" size="xl" @click="loginByGoogle"></UButton>
                </div>
            </div>
        </div>
    </div>
